...
const SORT_CONFIGS: Record<SortKey, { label: string; accessor: (m: ProtocolMetrics) => number | string; align: 'left' | 'right' }> = {
  name: { label: 'Protocol', accessor: (m) => m.name, align: 'left' },
  capital: { label: 'Capital at Risk', accessor: (m) => m.totalCapitalAtRisk, align: 'right' },
  admins: { label: 'Admins', accessor: (m) => m.adminCount, align: 'right' },
  eoaAdmins: { label: 'EOAs', accessor: (m) => m.eoaAdminCount, align: 'right' },
  deps: { label: 'Deps', accessor: (m) => m.dependencyCount, align: 'right' },
  functions: { label: 'Perm. Fns', accessor: (m) => m.permissionedFunctionCount, align: 'right' },
  contracts: { label: 'Contracts', accessor: (m) => m.contractCount, align: 'right' },
  capitalPerAdmin: { label: 'Capital/Admin', accessor: (m) => m.capitalPerAdmin, align: 'right' },
}

export function ComparisonTable({ metrics, onSelectForCompare, selectedSlugs }: ComparisonTableProps) {
  ...
  const maxFns = Math.max(...metrics.map((m) => m.permissionedFunctionCount), 1)
                ...
                >
                  ...
                  <td className="px-3 py-3 text-right">
                    <span
                      className={clsx(
                        ...
                        m.eoaAdminCount > 0 ? 'text-status-red' : 'text-status-green',
                      )}
                    >
                      {m.eoaAdminCount}
                    </span>
                  ...
                  <td className="px-3 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <span className="tabular-nums">{m.dependencyCount}</span>
                    </div>
                  </td>
                  <td className="px-3 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
