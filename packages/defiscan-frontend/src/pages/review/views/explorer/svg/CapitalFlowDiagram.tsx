...
export function CapitalFlowDiagram({ review }: CapitalFlowDiagramProps) {
  ...
  // For each admin, find which fund holders their functions touch directly
  ...
  topAdmins.forEach((admin, adminIdx) => {
    ...
    for (const fn of admin.functions) {
      // Check if the function's contract is a fund holder
  ...
  return (
    ...
    >
      ...
      {topAdmins.map((admin, i) => {
        ...
        return (
          <g key={`fn-${admin.address}`}>
            ...
            {/* Arrow from admin to function circle */}
            <line
              x1={adminColX + boxW}
              y1={y + boxH / 2}
              x2={fnColX + 26}
      ...
      <g transform={`translate(20, ${svgHeight - 20})`}>
        ...
        <text x="274" y="8" fill="#6B7280" fontSize="9">Capital at risk</text>
      </g>
