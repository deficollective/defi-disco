...
interface NarrativeCodeAndAuditsProps {
...
export function NarrativeCodeAndAudits({
  review,
}: NarrativeCodeAndAuditsProps) {
      ...
      {proxyContracts.length > 0 && (
        <div className="rounded-xl border-l-4 border-status-amber/40 bg-status-amber/5 px-6 py-4 mb-8">
          ...
          <p className="text-sm text-text-secondary leading-relaxed">
            ...
            {proxyContracts.map((c) => c.proxyType).join(', ')}), which means
            their behavior can be changed by whoever controls the upgrade
            mechanism. Upgradeable contracts provide flexibility for bug fixes
            but introduce centralization risk.
          </p>
        </div>
      )}
              ...
              {columns.map((col) => (
                <td key={col.field} className="px-4 py-2 text-text-primary">
                  {col.format === 'address' ? (
                    ...
                    row[col.field] ? (
                      <Badge variant="purple">{String(row[col.field])}</Badge>
                    ) : (
                      <span className="text-text-muted">-</span>
                    )
                  ) : (
