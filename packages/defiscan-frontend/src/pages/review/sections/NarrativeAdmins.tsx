...
interface NarrativeAdminsProps {
  review: CompiledReview
}

export function NarrativeAdmins({ review }: NarrativeAdminsProps) {
    ...
    <div>
      <p className="text-lg text-text-secondary leading-relaxed max-w-3xl mb-8">
        ...
        {totals.totalTokenValueAtRisk > 0 && (
          <>
            {' '}and{' '}
            <UsdValue value={totals.totalTokenValueAtRisk} variant="token" /> in
            protocol tokens
          </>
        )}
        .
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {humanControlled.map((admin) => (
              <NarrativeAdminCard key={admin.address} admin={admin} />
            ))}
          </div>
        </div>
      )}
...
function NarrativeAdminCard({ admin }: { admin: CompiledAdmin }) {
          ...
          {admins.map((admin) => (
            ...
            >
              ...
              <p className="mt-1 text-xs text-text-muted">
                ...
                {admin.totalDirectCapital > 0 && (
                  <>
                    {' '}-- <UsdValue value={admin.totalDirectCapital} variant="capital" className="text-xs" />
                  </>
                )}
              </p>
            </div>
