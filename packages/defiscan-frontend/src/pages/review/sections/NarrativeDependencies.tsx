...
interface NarrativeDependenciesProps {
  review: CompiledReview
}

export function NarrativeDependencies({ review }: NarrativeDependenciesProps) {
  ...
  return (
    <div>
      ...
      {/* "What does this mean for me?" callout */}
        ...
        {entityGroups.map(([entity, deps]) => (
          <div key={entity ?? '__ungrouped'}>
            ...
            <div className="space-y-3">
              {deps.map((dep) => (
                <NarrativeDependencyCard key={dep.address} dependency={dep} />
              ))}
            </div>
          </div>
        ))}
...
function NarrativeDependencyCard({
  dependency,
}: {
