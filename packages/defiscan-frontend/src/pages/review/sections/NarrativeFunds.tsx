...
export function NarrativeFunds({ review }: NarrativeFundsProps) {
  ...
  return (
    <div>
      ...
      {/* Horizontal bar chart -- visual metaphor for fund distribution */}
      {chartData.length > 1 && (
      ...
      <div className="space-y-4">
        {funds.map((fund) => (
          <NarrativeFundCard key={fund.address} fund={fund} />
        ))}
      </div>
    </div>
  )
...
function NarrativeFundCard({ fund }: { fund: CompiledFundHolder }) {
  const totalValue =
