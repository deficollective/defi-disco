...
export function ContractsTable({ review }: ContractsTableProps) {
  ...
  const permissionedCount = new Set(functions.map((f) => f.contractAddress)).size

  const filters: { label: string; value: Filter; count: number }[] = [
    { label: 'All', value: 'all', count: contracts.length },
    { label: 'Permissioned', value: 'permissioned', count: permissionedCount },
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-bg-muted">
              ...
              <SortHeader field="proxy" label="Proxy" current={sortField} dir={sortDir} onClick={handleSort} />
              <th className="px-4 py-2 font-medium text-text-secondary text-left">Entity</th>
              <th className="px-4 py-2 font-medium text-text-secondary text-center">Flags</th>
              <SortHeader field="functions" label="Perm. Fn" current={sortField} dir={sortDir} onClick={handleSort} className="text-right" />
            </tr>
          </thead>
          <tbody>
            {filtered.map((contract) => (
              <ContractRow
                ...
                functions={functions.filter((f) => f.contractAddress === contract.address)}
      ...
      >
        <td className="px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            <span className={`font-medium ${fnCount > 0 ? 'text-text-primary' : 'text-text-secondary'}`}>
              {contract.name}
            </span>
          </div>
        </td>
        ...
        <td className="px-4 py-2 text-center">
          <div className="flex items-center justify-center gap-1">
            {contract.isExternal && (
              <span className="text-xs px-1.5 py-0.5 rounded bg-status-amber/10 text-status-amber">Ext</span>
            )}
            {contract.isGovernance && (
              <span className="text-xs px-1.5 py-0.5 rounded bg-status-green/10 text-status-green">Gov</span>
            )}
          </div>
        </td>
        <td className="px-4 py-2 text-right font-medium text-text-primary">
          {fnCount > 0 ? fnCount : <span className="text-text-muted">-</span>}
        </td>
      </tr>
      {expanded && functions.length > 0 && (
        <tr>
          <td colSpan={6} className="px-0 py-0 bg-bg-muted/50 border-b border-border">
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {functions.map((fn) => {
                    ...
                    return (
                      <tr key={fn.functionName} className="border-t border-border/30">
                        <td className="py-1 font-mono text-text-primary">{fn.functionName}()</td>
                        <td className="py-1">
                        ...
                        <td className="py-1 text-text-secondary">
                          {fnAdmins.length > 0 ? fnAdmins.join(', ') : '-'}
                        </td>
                      </tr>
                    )
                  })}
