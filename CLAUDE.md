...
- Call-Graph Edge Overrides (the durable fix for the permission over-flare limitation in `permissions.md`. Researcher-authored rules that add/remove edges from the enhanced graph, persisted per project in `call-graph-overrides.json`, **re-applied on every recompute** (incl. the CI monitor). Backend `callGraphOverrides.ts` = file CRUD (`getCallGraphOverrides`/`updateCallGraphOverrides`/`getEdgeOverrideRules`) + a pure **extensible rule engine** `applyEdgeOverrides`: rules are a discriminated union, each `type` has one handler in `RULE_HANDLERS`, so **adding a rule = add a variant + one handler entry**. Initial rule set: `addEdge`/`removeEdge` (single edges keyed by node ids) and `removeOutgoing`/`removeIncoming` (all edges leaving/arriving at a function `address.function` or whole `address` contract, optional `edgeType` filter e.g. `'permission'`). Applied as the final step of `buildEnhancedGraph` — the single chokepoint feeding capital/governance/dependency/mitigation analysis (all 4 callers load via `getEdgeOverrideRules`). Keyed on stable node-id identity so rules survive call-graph regeneration; `applyEdgeOverrides` returns `unmatchedRuleIds` (rules matching nothing) for stale-cut detection. Endpoints `GET`/`PUT /api/projects/:project/call-graph-overrides` (PUT body = rules array; respects `--readonly`). Frontend mirrors the engine in `callgraph/rules.ts` (`applyRulesToCallEdges`) and applies it over the **raw** edge set the `enhanced-graph-edges` endpoint now returns (alongside `appliedRules`/`unmatchedRuleIds`). **Edge editing is sidebar-driven** (DetailSidebar Node tab = the edge editor): a selected node's Outgoing/Incoming edges are **reversible on/off toggles** (toggle off → `removeEdge`, row stays struck-through with restore; bulk-suppressed edges show a "by rule" chip → Rules tab; added edges show an "added" pill + remove), an inline **"＋ add outgoing edge"** form (searchable target + call/dependency type) creates `addEdge`, and section headers carry **"✂ owns / ✂ all"** → `removeOutgoing`/`removeIncoming`. `buildCallgraph` returns both `edges` (post-override) + `rawEdges` (pre-override) so the sidebar can render suppressed edges; mutations are optimistic. The standalone Edges tab was removed; the **Rules tab** is the ledger (delete + stale ⚠). `RuleBase.scope?: 'forward'|'backward'|'both'` is reserved (not yet honored).)
