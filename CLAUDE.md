...
- DeFiScan Frontend (static React app, Vercel deployment, shareable report view, TVS metric, mitigation badges in report cards). **Three-timestamp model on `CompiledReview`**: `publishedAt` (first time `review-config.json` was ever created — preserved forever across regenerations via `writeReviewConfig` chokepoint, plus out-of-band `/tmp/review-config-$0-publishedAt.txt` preservation in the `/generate-review` skill which bypasses the backend API), `updatedAt` (last researcher edit to `review-config.json` / `resources.json` / `governance.json` — bumps via `writeReviewConfig`, `updateResources`/`updateAudits` with `bumpLastModified: true`, or `governance.json` filesystem mtime; `updateLinesOfCode` explicitly calls `writeResourcesFile(..., false)` so compile-side LoC recounts do NOT bump `updatedAt`), and `compiledAt` (live on-chain data freshness — sourced from `discovered.json.timestamp` in `reviewCompiler.ts`, NOT wall-clock compile time, so researcher-triggered recompiles without a fresh discovery cycle keep it frozen). `HeroSection` "Updated" shows `updatedAt`; `TimestampsFooter` (standalone component rendered as the final section of `ReportView`, below Protocol Activity) shows three info pills (Published / Last updated / On-chain data); `ActivityView` "Last Verified" keeps `compiledAt` (semantics match); `LandingPage` ranks "recently updated" by `updatedAt`. Gallery cards derive UPDATED status from `updatedAt` (< 7 days = UPDATED badge). `ReviewConfig.publishedAt?: string` is optional on the type but backfilled in `getReviewConfig` (falls back to `lastModified`) and preserved in `writeReviewConfig` by reading the existing file first. `resources.json` carries its own optional `lastModified` field; `governance.json` uses filesystem mtime (no schema change). All three timestamps are assembled at the `reviewCompiler.buildCompiledReview` return site, taking the max of `reviewConfig.lastModified` / `getResourcesLastModified` / `getGovernanceLastModified` for `updatedAt`
  ...
  - Gallery Page (`/gallery` — card grid of all protocols with radar chart, filters, pagination, status badge. **Status badges**: ACTIVE = default; UPDATED = `review.updatedAt` is less than 7 days ago (researcher-edit timestamp, not on-chain activity). Hover tooltip explains semantics.)
