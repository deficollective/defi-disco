...
- Radar Scoring (`deriveRadarData()` in `packages/defiscan-frontend/src/utils/radar.ts` — 5-axis scores for ADMIN CONTROL / DEPENDENCIES / ACCESS / VERIFIABILITY / GOVERNANCE driving the Report hero, Gallery radar charts, and Landing Trust Posture; full per-axis scoring formulas in the scoring-and-review doc. ADMIN CONTROL and DEPENDENCIES use continuous formulas — worst-admin risk from type/threshold/delay/fund-share, and worst-dependency-exposure with a concave tail — not discrete tiers. Multisig threshold/size come from structured `multisigThreshold`/`multisigSize` fields on `AdminEntry`, populated in `ProjectAnalysis.getAdmins()` from the Gnosis Safe discovery entry's `$threshold`/`$members`)
