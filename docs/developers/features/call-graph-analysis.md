...
- **Rule types:** `addEdge` / `removeEdge` (existence — single callgraph edges, keyed by node ids) and `setEdgeScope` (single) / `setOutgoingScope` / `setIncomingScope` (bulk by node, optional `edgeType` filter) which set the `scope` attribute. `RuleBase` carries `id`, `note?`, `enabled?`.
