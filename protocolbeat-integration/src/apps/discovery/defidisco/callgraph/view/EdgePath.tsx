...
export function EdgePath({
  edge, fromX, fromY, toX, toY,
  hovered, onPath, dimmed, showLabel,
  onHover, onClick,
}: Props): JSX.Element {
  ...
  const stroke = onPath ? 'var(--aux-cyan, #1c92a8)' : COLOR_BY_KIND[edge.kind] ?? COLOR_BY_KIND.internal
      ...
      {showLabel && labelText && (
        <>
          ...
          <text
            ...
            y={my + 2.5}
            textAnchor="middle"
            fill={onPath ? 'var(--aux-cyan, #1c92a8)' : 'var(--coffee-200, #E0D4F0)'}
            style={{ font: '9.5px ui-monospace, Menlo, monospace', pointerEvents: 'none' }}
          >
            {labelText}
          </text>
        </>
