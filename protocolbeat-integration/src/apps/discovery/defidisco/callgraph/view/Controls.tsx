...
export function Controls({
  ...
  onClearStart, onReset,
  trail, nodes, onSelectNode,
}: Props): JSX.Element {
  return (
    <div className="flex items-center gap-2 overflow-x-auto border-coffee-500 border-t bg-coffee-700 px-3 py-2 font-mono text-[11px]">
      ...
      <Group label="layout">
        {(['tree', 'lanes', 'compact'] as LayoutMode[]).map((m) => (
          <PillButton key={m} on={layoutMode === m} onClick={() => setLayoutMode(m)}>
      ...
      <div
        className={clsx(
          'flex max-w-[40%] items-center gap-1 overflow-x-auto rounded border border-coffee-600',
          ...
          trail.map((id, i) => {
            ...
            return (
              <span key={`${id}-${i}`} className="flex items-center gap-1">
                ...
                >
                  <span className="text-coffee-400">{node?.contractName}{fn ? '.' : ''}</span>
                  {fn ?? ''}
                </button>
              </span>
            )
      ...
      <PillButton on={editMode} onClick={() => setEditMode(!editMode)}>
        {editMode ? '● edit' : '○ edit'}
      </PillButton>
      <PillButton onClick={onClearStart}>↺ new trace</PillButton>
      <PillButton onClick={onReset} title="Clear user-added/removed edges and notes">
        reset
      </PillButton>
    </div>
  )
...
function Group({
  ...
  return (
    <div className="flex items-center gap-1.5 border-r border-coffee-600 pr-3 last:border-r-0">
...
function PillButton({
  ...
  return (
    <button
      ...
      title={title}
      className={clsx(
        'inline-flex h-7 items-center gap-1.5 whitespace-nowrap rounded border px-2.5',
